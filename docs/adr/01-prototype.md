...
## D12. 作品の公開モデル（status 一本化 + ポートフォリオ主導の掲載）

- **文脈**: 当初は作品単位 `is_public`（掲載トグル）＋ `isDraft`（下書き）＋ `status`(draft/published) と公開系の概念が重複し、「登録済みなのに非公開で検索に出ない」等の分かりにくさが出た。CMS としての素直なモデルに整理したい
- **決定**:
  - 作品のライフサイクルを **`status` enum = `draft` / `published` / `archived`** に一本化。**`isDraft` / `isPublic` は廃止**
  - **公開（検索・公開ページに出る）＝ `status='published'`**。`draft`/`archived` は非公開
  - **ポートフォリオ掲載は作品側で持たず、ポートフォリオ側で選ぶ**（`/portfolio/edit`）。公開ポートフォリオ `/p/[slug]` は「published かつポートフォリオに選択された作品」を選択順で表示。published だが未選択の作品は検索・直リンクでは見えるがポートフォリオ・トップには出ない
  - 編集はすべて**自動保存**。明示の「保存」ボタンは持たず、状態遷移（status 変更）だけがユーザー操作。`published` 化時はタイトル必須
  - 販売状態（販売中/取引中/販売済 等）は別概念。**本フェーズではカラムを持たない**（項目未確定・将来追加）
- **帰結**:
  - DB: `artwork.is_draft` / `artwork.is_public` を撤去、`artwork_status` enum に `archived` を追加。ポートフォリオ選択を表す join（作品×ポートフォリオの掲載＋順序）を追加
  - api: 可視判定・検索・公開ポートフォリオを `status='published'`（＋ポートフォリオ選択）基準に変更。ポートフォリオ編集用エンドポイントを追加
  - web: 作品フォームから掲載チェック・保存ボタンを撤去（status select で公開/非公開、自動保存）。`/portfolio/edit` を新設
- **代替案**: `is_public` を作品に残す（作品が掲載可否を自己管理）→ ポートフォリオがキュレーションを担う設計と二重化するため不採用（本 ADR が `02` の旧「作品単位 is_public」記述を更新する）
