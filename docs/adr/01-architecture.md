...
- **文脈**: 作品の「下書き／公開」と「ポートフォリオ掲載」を、CMS として迷いの少ない形で表現したい
- **決定**:
  - 作品のライフサイクルを **`status` enum = `draft` / `published` / `archived`** で表す
  - **公開（検索・公開ページに出る）＝ `status='published'`**。`draft`/`archived` は非公開
  - **ポートフォリオ掲載は `/portfolio/edit` で選ぶ**（掲載＋順序を表す `portfolio_item` で保持）。公開ポートフォリオ `/p/[slug]` は「published かつポートフォリオに選択された作品」を選択順で表示。published で未選択の作品は検索・直リンクでは見えるがポートフォリオ・トップには出ない
...
- **帰結**:
  - DB: `artwork.status`（enum）と `portfolio_item`（作品×ポートフォリオの掲載＋順序）で構成
  - api: 可視判定・検索・公開ポートフォリオは `status='published'`（＋ポートフォリオ選択）基準。ポートフォリオ編集用エンドポイントを持つ
  - web: 作品フォームは status select（公開/非公開）と自動保存。掲載は `/portfolio/edit`
- **代替案**: 作品に `is_public`（掲載可否）を持たせ作品側で自己管理 → ポートフォリオがキュレーションを担う設計と二重化するため不採用
