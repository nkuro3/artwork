...
- [x] **Ed dev storage 分離 + フォーク追従** `@api` — Neon の dev フォークに storage を整合。R2 はブランチ非対応のため `apps/api/scripts/sync-dev-storage.ts`（`bun run sync-dev-storage`）で「dev DB が参照する r2_key を Production バケット → dev バケットへ S3 CopyObject」コピー（冪等・HEAD でスキップ・dev→prod 書込み無し）。✅ dev バケット `artwork-images-dev` 作成、R2 トークン両バケット可、CopyObject 疎通（200）。`.dev.vars` を dev に切替（本番は [vars]）。✅ dev 画像ドメイン `images-dev.artwork-store.jp` 接続 + 配信/変換を実画像で検証（raw 200・w=100 200 image/jpeg）。dev 環境（DB/R2/画像配信）本番同等で完成。

- [x] **E1 🔒 プロビジョニング/デプロイ** — ✅ Neon/R2/Secret/DNS 委任/`wrangler.toml` ルート整備済。✅ **R2 カスタムドメイン `images.artwork-store.jp` 接続 + Image Resizing 有効化（実画像で 200 変換確認、format=auto で JPEG 化）**。✅ wrangler.toml `[vars]`（BETTER_AUTH_URL/R2_BUCKET_NAME/IMAGE_BASE_URL）。✅ **②本番シークレット投入（DATABASE_URL=Production/他は共有）→ ③`wrangler deploy` 成功・api 本番稼働**（`artwork-store.jp/api/*` ルート、`/api/health` 200・`/api/portfolio/x` 404=Neon Production 接続OK・`/api/auth/ok` 200）。修正: wrangler.toml の `routes` は TOML 仕様上 `[vars]` の前に置く必要（後ろだと vars.routes に吸収されルート未適用＝503 だった）。✅ **④web OpenNext セットアップ + デプロイ成功**（`artwork-web`、ルート `artwork-store.jp/*`、R2 キャッシュ/ASSETS/API_URL バインディング）。**本番フルスタック稼働確認**: `/` `/login` `/signup` 200、`/api/health` 200（api 優先維持）、`/p/:slug` 404=web SSR→api→Neon 往復成立（同一オリジン連携 OK）。E1 完了。
