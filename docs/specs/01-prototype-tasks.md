...
- [~] **Ed dev storage 分離 + フォーク追従** `@api` — Neon の dev フォークに storage を整合。R2 はブランチ非対応のため `apps/api/scripts/sync-dev-storage.ts`（`bun run sync-dev-storage`）で「dev DB が参照する r2_key を Production バケット → dev バケットへ S3 CopyObject」コピー（冪等・HEAD でスキップ・dev→prod 書込み無し）。✅ dev バケット `artwork-images-dev` 作成、R2 トークンを両バケット可に更新、CopyObject 疎通確認（200）。`.dev.vars` の R2_BUCKET_NAME/IMAGE_BASE_URL を dev に切替（本番は wrangler.toml [vars] が prod 値）。⬜ 手順3: dev 画像ドメイン `images-dev.artwork-store.jp` 接続 + Transformations（あなた）。
